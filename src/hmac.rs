...
pub trait Hash {
    ...
    fn hmac(key: &[u8], message: &[u8]) -> Vec<u8> {
        ...
        if key.len() < Self::block_size() {
            key = Cow::Owned(key.iter().cloned().chain(iter::repeat(0u8).take(Self::block_size() - key.len())).collect());
        }
...
mod tests {
    ...
    fn test_hmac_empty_md5() {
        assert_eq!(Md5::hmac(b"", b""), vec![0x74, 0xe6, 0xf7, 0x29, 0x8a, 0x9c, 0x2d, 0x16, 0x89, 0x35, 0xf5, 0x8c, 0x00, 0x1b, 0xad, 0x88]);
    }

    #[test]
    fn test_hmac_md5() {
        assert_eq!(Md5::hmac(b"key", b"The quick brown fox jumps over the lazy dog"), vec![0x80, 0x07, 0x07, 0x13, 0x46, 0x3e, 0x77, 0x49, 0xb9, 0x0c, 0x2d, 0xc2, 0x49, 0x11, 0xe2, 0x75]);
    }
}
